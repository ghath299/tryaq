...
export default function LocationPermissionScreen() {
  ...
  const completeWithLocation = async (
    ...
    // ابحث عن الاسم العربي للمحافظة
    const province = provinces.find((p) => p.nameEn === nameEn);
    const provinceAr = province?.nameAr || nameEn;

    if (coords) {
      // أرسل الإحداثيات الحقيقية
  ...
  const handleRequestPermission = async () => {
    ...
    try {
      if (Platform.OS === "web") {
        await completeWithLocation("Baghdad");
        return;
      }
      ...
      await completeWithLocation(governorate, {
      ...
      >
        <View style={styles.header}>
          ...
          <ThemedText type="body" style={[styles.subtitle, { color: theme.textSecondary }]}>
            نحتاج موقعك لتحديد المحافظة وعرض الصيدليات والأطباء القريبين منك.
          </ThemedText>
        </View>

        {showDeniedMessage ? (
          <View style={[styles.deniedBox, { backgroundColor: "rgba(255,59,48,0.08)", borderColor: "#FF3B30" }]}>
          ...
          >
            ...
            >
              ...
              <ThemedText type="body" style={styles.buttonText}>
                {isRequesting ? "جاري تحديد موقعك..." : "السماح بالوصول للموقع"}
              </ThemedText>
            </LinearGradient>
          </Pressable>
...
const styles = StyleSheet.create({
  ...
  modalCard: {
    borderRadius: BorderRadius.md, maxHeight: "70%",
